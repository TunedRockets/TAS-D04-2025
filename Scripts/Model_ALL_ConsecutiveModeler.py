...
def tow_visualizer(tows: list[pd.DataFrame], y_intended: list, name: str, ideal: bool):
    ...
    if not all(isinstance(df, pd.DataFrame) for df in tows):
    ...
    for i in range(len(y_intended)):
        ...
        plt.plot([tows[i]['x'].iloc[0], tows[i]['x'].iloc[0]], [tows[i]['center_CAM'].iloc[0] - 0.5 * tows[i]['width_LLS_B'].iloc[0], tows[i]['center_CAM'].iloc[0] + 0.5 * tows[i]['width_LLS_B'].iloc[0]], linestyle='solid', color='black')
        plt.plot([tows[i]['x'].iloc[-1], tows[i]['x'].iloc[-1]], [tows[i]['center_CAM'].iloc[-1] - 0.5 * tows[i]['width_LLS_B'].iloc[-1], tows[i]['center_CAM'].iloc[-1] + 0.5 * tows[i]['width_LLS_B'].iloc[-1]], linestyle='solid', color='black')

    if ideal == True:
        #plot the ideal path (just a rectangle)
